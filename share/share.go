...
func recoverAdditiveLarge128(nums ...*Uint128) *Uint128 {
        sum := MakeUint128(0, 0)
        for _, num := range nums {
                sum.AddInPlace(num)
        }
...
func PtxtMulRSSInPlace(s *share, v uint64) {
        s.First *= v
        s.Second *= v
}

func PtxtMulRSSInPlace128(s *Share128, v uint64) {
        if (v >> 63) & 1 == 1 {
                panic("Warning: this method gives the wrong answer on negative values. Cast ptxt to Uint128 instead.")
        }
        s.First.Mul64InPlace(v)
        s.Second.Mul64InPlace(v)
}
...
func largePtxtMulAddRSS128(s *Share128, v *Uint128, dst *Share128) {
        dst.First.MulAddInPlace(&s.First, v)
        dst.Second.MulAddInPlace(&s.Second, v)
}
...
func recoverRSS128From2(s0, s1 *Share128) uint64 {
        if (!s0.First.Equals(&s1.Second)) {
		s0.First.Print()
		s0.Second.Print()
		s1.First.Print()
		s1.Second.Print()
                panic("Bad RSS share in recoverRSS128From2")
        }
	...
	interm.AddInPlace(&s1.First)
        ...
        if !CheckUint64(interm) {
                interm.Print()
                panic("recoverRSS128From2: secret shared value does not fit in 64 bits")
        }
