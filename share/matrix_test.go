...
func TestMatPtxtMul(test *testing.T) {
	...
	nRows := uint64(100)
	nCols := uint64(100)
	pool := InitPRGPoolFromCapacity(uint64(100), NUM_VCPUS)
        perf := new(PerfLog)
...
func testPowerIt(test *testing.T, saveTrunc bool) {
	...
	// Test that accurately factors a rank-2 matrix read from a file
	U = ReadMatrixFromFile("test_matrix/matrix1.csv")
	if b, _, _ := runPowerIt(U, nil, params, false, false, ""); !b {
		test.Fail()
		panic("Bad event!")
	}
